...
After synthesis, the coordinator should ask whether you want an optional `summary.html`.
...
The optional summary is generated from `synthesizer.md` and may consult `discussion.md` for short quotes or attribution. The synthesizer can also provide a `## Summary UI Data` JSON payload in `synthesizer.md` so tooling can render the page directly. The model should fill only that JSON payload; the HTML should be generated by `council summary` from the canonical template. The summarizer's job is translation and presentation only:
